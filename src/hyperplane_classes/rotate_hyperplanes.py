...
def load_data_from_csv(file_path):
    ...
    with open(file_path, mode='r') as file:
        ...
        for row in reader:
            features.append([float(row[0]), float(row[1])])  # only first two features matter
            labels.append(int(row[len(row)-1]))  # to work for any number of features
...
def func(rotation_degrees: int):
    # initial_seed = 2
    # dataset = synth.Hyperplane(seed=initial_seed, n_features=2, noise_percentage=0)
    #
    # original_features = []
    # original_labels = []
    #
    # for x, y in dataset.take(30):
    #     original_features.append([x[0], x[1]])
    #     original_labels.append(y)
    #
    # original_features = np.array(original_features)
    # original_labels = np.array(original_labels)

    csv_file = "../hyperplanes_with_redundancy/200k_samples_hyperplane_red_5/200k_boundary_rotated_0_hyperplane.csv"
    ...
    #     if rotated_labels[idx] == original_labels[idx]:
    #         rotated_labels[idx] = 1 - original_labels[idx]
    #     else:
    #         rotated_labels[idx] = original_labels[idx]
